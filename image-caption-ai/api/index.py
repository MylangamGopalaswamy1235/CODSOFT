...
def get_openai_client():
    ...
    if not api_key:
        raise RuntimeError("OpenAI API key is missing. Set OPENAI_API_KEY in your environment variables.")
    if client is None:
...
def predict():
    ...
    try:
        ...
        if "api key" in lowered or "api_key" in lowered or "authentication" in lowered:
            return jsonify({"error": "OpenAI API key is invalid or missing. Set OPENAI_API_KEY in your Vercel environment variables."}), 500
